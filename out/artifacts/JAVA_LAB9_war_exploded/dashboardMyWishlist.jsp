...
<div class="wrapper">
    ...
    <div class="mainContent" id="main">
        ...
        <%
            ...
            ResultSet rs = stm.executeQuery("select wishlist.status, event.name, event.id, event.description, event.startDate, event.startTime" +
                    " from wishlist join event on event.id = wishlist.eventid where wishlist.studentid = '"+studentid+"'");
            while(rs.next())
            {
        %>
        <%--      myeventcontent--%>
                ...
                <div class="event">
                    <div style="display: flex;">
                        ...
                        <div>
                            ...
                            <div class="eDateTime"><h6>Status :  <%= rs.getString("wishlist.status")%></h6></div>
                        </div>
                    </div>
                    <a href="dashboardMyWishlistRemove.jsp?eid=<%= rs.getString("event.id")%>"><button class="readMore">Remove</button></a>
                    <a href="eventJoinPage.jsp?eid=<%= rs.getString("event.id")%>"><button class="readMore">Register now</button></a>
                </div>
            </div>
        </div>
        <%
